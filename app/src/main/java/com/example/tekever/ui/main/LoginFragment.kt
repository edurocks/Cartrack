...
class LoginFragment : Fragment(), View.OnClickListener {
    ...
    private fun observeResult() {
        mViewModel.mUser.observe(viewLifecycleOwner, { user ->
            user.let {
                if (it.username != mUsername?.editText?.text.toString()
                    && it.password != mPassword?.editText?.text.toString()){
                    // erro nao existe na bd
                    Snackbar.make(requireView(), "User don't exist in the database",
                            Snackbar.LENGTH_LONG).show()
                }else{
                    findNavController().navigate(R.id.action_loginFragment_to_usersDetailFragment)
                }
            }
        })
    ...
    override fun onClick(v: View?) {
        mViewModel.getAndValidateUser(mUsername?.editText?.text.toString(),
                                      mPassword?.editText?.text.toString(),
